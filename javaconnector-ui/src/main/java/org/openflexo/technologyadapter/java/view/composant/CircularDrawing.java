...
public class CircularDrawing extends DrawingImpl<JAVAGraph> {
	...
	public void init() {
		...
		nodeRepresentation.setBackground(getFactory().makeColoredBackground(
				Color.red));
		edgeRepresentation = getFactory().makeConnectorGraphicalRepresentation(
				ConnectorType.CURVE);

		FGECircle circle1 = new FGECircle(new FGEPoint(310, 310), 100,
				Filling.NOT_FILLED);
		FGECircle circle2 = new FGECircle(new FGEPoint(310, 310), 200,
				Filling.NOT_FILLED);
		...
		final DrawingGRBinding<JAVAGraph> graphBinding = bindDrawing(
				JAVAGraph.class, "graph", new DrawingGRProvider<JAVAGraph>() {
					...
					public DrawingGraphicalRepresentation provideGR(
							JAVAGraph drawable, FGEModelFactory factory) {
						return graphRepresentation;
					}
				});
		final ShapeGRBinding<JAVAGraphNode> nodeBinding = bindShape(
				JAVAGraphNode.class, "node",
				new ShapeGRProvider<JAVAGraphNode>() {
					@Override
					public ShapeGraphicalRepresentation provideGR(
							JAVAGraphNode drawable, FGEModelFactory factory) {
						return nodeRepresentation;
					}
				});
		final ConnectorGRBinding<JAVAEdge> edgeBinding = bindConnector(
				JAVAEdge.class, "edge", nodeBinding, nodeBinding, graphBinding,
				new ConnectorGRProvider<JAVAEdge>() {
					@Override
					public ConnectorGraphicalRepresentation provideGR(
							JAVAEdge drawable, FGEModelFactory factory) {
						return edgeRepresentation;
					}
				});

		final GeometricGRBinding<JAVAGraph> circle1Binding = bindGeometric(
				JAVAGraph.class, "circle1",
				new GeometricGRProvider<JAVAGraph>() {
					@Override
					public GeometricGraphicalRepresentation provideGR(
							JAVAGraph drawable, FGEModelFactory factory) {
						return circle1GR;
					}
				});
		...
		nodeBinding.addToWalkers(new GRStructureVisitor<JAVAGraphNode>() {
			...
			public void visit(JAVAGraphNode node) {
				...
				for (JAVAEdge edge : node.getInputEdges()) {
					drawConnector(edgeBinding, edge, edge.getStartNode(),
							edge.getEndNode(), node.getGraph());
				}
