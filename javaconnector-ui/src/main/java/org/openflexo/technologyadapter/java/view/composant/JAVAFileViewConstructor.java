...
public class JAVAFileViewConstructor {
	...
	public CircularDrawing makeDrawing() {
		...
		JAVAGraphNode node = new JAVAGraphNode(fileModel.getName(), graph,
				fileModel);
	...
	private void completeDrawing(JAVAGraph graph,
			...
			List<JAVAClassOrInterfaceModel> classList, int size, int height) {
		...
		if (size > 0 && height < 1) {
			for (int i = 0; i < size; i++) {
				...
				for (JAVAClassOrInterfaceModel innerClass : classModel
						.getInnerClasses()) {
					JAVAGraphNode node = new JAVAGraphNode(
							innerClass.getName(), graph, innerClass);
					parent.connectTo(node);
					newClassList.add(innerClass);
				}
				for (JAVAFieldModel fieldModel : classModel.getFields()) {
					JAVAGraphNode node = new JAVAGraphNode(
							fieldModel.getName(), graph, fieldModel);
					parent.connectTo(node);
					newGraphNodeList.add(node);
				}
				for (JAVAMethodModel methodModel : classModel.getMethods()) {
					JAVAGraphNode node = new JAVAGraphNode(
							methodModel.getName(), graph, methodModel);
					parent.connectTo(node);
					newGraphNodeList.add(node);
				}
			}
			completeDrawing(graph, newGraphNodeList, newClassList,
					newClassList.size(), ++height);
		}
